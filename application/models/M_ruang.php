...
class M_ruang extends CI_Model{
	...
	public function searchBetween($min, $max) {
		$query = $this->db->query("SELECT * FROM ruang WHERE kuota BETWEEN '$min' AND '$max'");
		return $query->result();
	}

	public function getRuangSearch($keyword) {
		$this->db->select('*');
		$this->db->from('ruang');
